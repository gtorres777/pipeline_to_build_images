...
def upload_dump(dump_name, server, port, user, path, ssh_key, dest_dump_name=None):
    """
    ...
    try:
        # logging.info("Server string: ", server_string)
        # Arguments: ('odoo@upgrade.odoo.com:/data',)
        run_command(
            [
                "rsync",
...
def download_dump(server, port, user, dump_path, dump_name, ssh_key):
    """
...
def dump_database(db_name, dump_name, core_count):
    ...
    # pg_dump -Fc -v -h odoo-db.clgnyaxunaji.us-east-1.rds.amazonaws.com -U sanahoria lasanahoria
    # JtnU3JC_-tRd9DNZ
    #  python3.7 upgrade.py test -d victorynk -t 14.0
    x = [
        "pg_dump",
        "--no-owner",
        "--format",
...
def get_db_contract(dbname):
    try:
        # psql --host=odoo-db.clgnyaxunaji.us-east-1.rds.amazonaws.com --port=5432 --username=postgres --password
...
def get_dump_basename_and_format(dump):
    ...
    """
    dump_ext = next((e for e in EXPECTED_DUMP_EXTENSIONS if dump.endswith(e)), None)
    if dump_ext:
...
def send_json_request(request, params):
    ...
    try:
        response = urlrequest.urlopen(req, timeout=REQUEST_TIMEOUT)
        info = response.info()
...
def get_logs(token, from_byte=0):
    ...
    response = urlrequest.urlopen(req, timeout=REQUEST_TIMEOUT).read().decode("utf-8")
    return response
...
def new_handler(fsm):
    ...
    if input_source == "db":
        ...
        dump_name = get_dump_name(dbname)
        dest_dump_name = None
    else:
        dump_name, dump_ext = fsm.get_context_data(("dump_filename", "dump_ext"))
...
def done_handler(fsm):
    ...
    (
        ...
        ("data_server_name", "data_server_user", "data_server_path", "no_restore")
    )
...
def parse_command_line():
    ...
    def add_upgrade_arguments(subparser):
        ...
        subparser.add_argument(
            "-i",
            "--dump",
            help="The database dump to upgrade (.sql, .dump, .sql.gz or .zip)",
        )
        subparser.add_argument(
            "-c",
            "--contract",
